...
if TYPE_CHECKING:
    from experionml.data.dataengines import DataEngine
...
class Sequence(Protocol[_T]):
    ...
    def __class_getitem__(cls, item: Any) -> Annotated[Any, Is]:
        """Obtém a anotação da sequência para tipo e conteúdo."""
        return Annotated[
            cls,
...
class MetadataDict(TypedDict, total=False):
    """Tipo de dicionário para o parâmetro `metadata`."""
...
class SPDict(TypedDict, total=False):
    """Tipo de dicionário para o parâmetro `sp`."""
...
class EngineDict(TypedDict, total=False):
    """Tipo de dicionário para o parâmetro `engine`."""
...
class HT(TypedDict, total=False):
    """Tipos do atributo `_ht` de Model."""
...
class Style(TypedDict):
    """Tipos para os estilos de plotagem."""
...
class EngineTuple(NamedTuple):
    ...
    def __repr__(self) -> str:
        """Exibe a representação como dicionário."""
        return self._asdict().__repr__()

    @property
    def data_engine(self) -> DataEngine:
        """Retorna o engine de dados."""
        from experionml.data import DATA_ENGINES
...
class PandasConvertible(Protocol):
    """Protocolo para qualquer objeto que possa ser convertido em pandas."""
...
class DataFrame(Protocol):
    """Protocolo para qualquer objeto que siga o protocolo de intercâmbio de dataframes."""
...
class SkScorer(Protocol):
    """Protocolo para os scorers do sklearn."""
...
class Estimator(Protocol):
    """Protocolo para estimadores do tipo sklearn."""
...
class Transformer(Estimator, Protocol):
    """Protocolo para transformadores do tipo sklearn."""
...
class Predictor(Estimator, Protocol):
    """Protocolo para preditores do tipo sklearn."""
...
class Model(Protocol):
    """Protocolo para todos os modelos."""
...
FeatureNamesOut: TypeAlias = (
    Callable[[Any, Sequence[str]], Sequence[str]] | Literal["one-to-one"] | None
)
...
if TYPE_CHECKING:
    ...
    YReturn: TypeAlias = (
        np.ndarray | pd.Series | pl.Series | pa.Array | md.Series | dd.Series | ps.Series
    )
else:
