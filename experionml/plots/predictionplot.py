...
class PredictionPlot(BasePlot, metaclass=ABCMeta):
    ...
    def plot_bootstrap(
        ...
        *,
        ...
        """Plota os escores de [bootstrapping][].
        ...
        experionml.run(["GNB", "LR"], metric=["f1", "recall"], n_bootstrap=5)
        experionml.plot_bootstrap()
        ...
        if all(m._bootstrap is None for m in models_c):
            raise ValueError(
                ...
                "run method,e.g., experionml.run(models=['LR', 'LDA'], n_bootstrap=5)."
            )
    ...
    def plot_calibration(
        ...
        *,
        ...
        """Plota a curva de calibração para um classificador binário.

        Classificadores bem calibrados são classificadores
        probabilísticos para os quais a saída do método `predict_proba`
        pode ser interpretada diretamente como um nível de confiança.
        Por exemplo, um classificador binário calibrado deve classificar
        as amostras de tal forma que, entre as amostras às quais atribui
        um valor de `predict_proba` próximo de 0,8, aproximadamente 80%
        realmente pertençam à classe positiva. Leia mais na
        [documentação][calibration] do sklearn.

        Esta figura mostra dois gráficos: a curva de calibração, em que
        o eixo x representa a probabilidade média predita em cada bin e
        o eixo y é a fração de positivos, isto é, a proporção de
        amostras cuja classe é a positiva em cada bin; e uma
        distribuição de todas as probabilidades preditas pelo
        classificador. Este gráfico está disponível apenas para modelos
        com método `predict_proba` em tarefas de classificação binária
        ou [multirrótulo][].

        !!! tip
            Use o método [calibrate][adaboost-calibrate] para calibrar o
            modelo vencedor.

        Parâmetros
        ----------
        models: int, str, Model, segment, sequence or None, default=None
            Modelos a plotar. Se None, todos os modelos são
            selecionados.

        rows: str, sequence or dict, default="test"
            Seleção de linhas sobre as quais calcular a métrica.

            - Se str: nome do conjunto de dados a plotar.
            - Se sequence: nomes dos conjuntos de dados a plotar.
            - Se dict: nomes dos conjuntos com a correspondente
              [seleção de linhas][row-and-column-selection] como valor.

        target: int or str, default=0
            Coluna alvo a considerar. Apenas para tarefas
            [multirrótulo][].

        n_bins: int, default=10
            Número de bins usados para a calibração. Mínimo de 5.
    ...
    def plot_confusion_matrix(
        ...
        *,
        ...
        experionml = ExperionMLClassifier(X, y, test_size=0.4)
        experionml.run(["LR", "RF"])
        experionml.lr.plot_confusion_matrix()  # For one model
        experionml.plot_confusion_matrix()  # For multiple models
        ```
    ...
    def plot_cv_splits(
        ...
        *,
        ...
        """Visualiza as divisões da validação cruzada.
        ...
        # Without groups
        experionml = ExperionMLClassifier(X, y, shuffle=False, n_rows=0.2, random_state=1)
        experionml.run("LR", metric=["f1", "auc"])
        experionml.lr.cross_validate(cv=4)
        experionml.plot_cv_splits()

        # With groups
        groups = choices(["A", "B", "C", "D"], k=X.shape[0])
        experionml = ExperionMLClassifier(X, y, metadata={"groups": groups}, n_rows=0.2, random_state=1)
        experionml.run("LR", metric=["f1", "auc"])
        experionml.lr.cross_validate(cv=4)
        experionml.plot_cv_splits()
        ...
        experionml.run("Croston", metric=["mape", "mse", "mae"])
        experionml.croston.cross_validate(cv=4)
        experionml.plot_cv_splits()
        ```
    ...
    def plot_det(
        ...
        *,
        ...
        """Plota a curva Detection Error Tradeoff.
    ...
    def plot_errors(
        ...
        *,
        ...
        """Plota os erros de predição de um modelo.
        ...
        experionml = ExperionMLRegressor(X, y)
        experionml.run(["OLS", "LGB"])
        ...
        for m in models_c:
            for child, ds in self._get_set(rows):
                ...
                from experionml.models import OrdinaryLeastSquares
    ...
    def plot_evals(
        ...
        *,
        ...
        experionml = ExperionMLClassifier(X, y, random_state=1)
        experionml.run(["XGB", "LGB"])
        experionml.plot_evals()
        ```
    ...
    def plot_feature_importance(
        ...
        *,
        ...
        """Plota a importância das features de um modelo.
        ...
        experionml = ExperionMLClassifier(X, y, random_state=1)
        experionml.run(["LR", "RF"])
        experionml.plot_feature_importance(show=10)
        ```
    ...
    def plot_forecast(
        ...
        *,
        ...
        """Plota as previsões dos modelos para a série temporal alvo.
        ...
        experionml.plot_forecast()
        experionml.plot_forecast(fh="train+test", plot_interval=False)

        # Forecast the next 4 years starting from the test set
        experionml.plot_forecast(fh=ForecastingHorizon(range(48)))
        ```
    ...
    def plot_gains(
        ...
        *,
        ...
        """Plota a curva de ganhos acumulados.
    ...
    def plot_learning_curve(
        ...
        *,
        ...
        """Plota a curva de aprendizado: pontuação versus número de amostras de treino.
        ...
        experionml = ExperionMLClassifier(X, y, random_state=1)
        experionml.train_sizing(["LR", "RF"], n_bootstrap=5)
    ...
    def plot_parshap(
        ...
        *,
        ...
        """Plota a correlação parcial dos valores SHAP.
        ...
        experionml = ExperionMLClassifier(X, y, random_state=1)
        experionml.run(["GNB", "RF"])
        experionml.rf.plot_parshap(legend=None)
        experionml.plot_parshap(columns=slice(5, 10))
        ```
    ...
    def plot_partial_dependence(
        ...
        *,
        ...
        """Plota a dependência parcial das features.
        ...
        experionml = ExperionMLClassifier(X, y, random_state=1)
        experionml.run(["LR", "RF"])
        experionml.plot_partial_dependence(kind="average+individual", legend="upper left")
        experionml.rf.plot_partial_dependence(columns=(3, 4), pair=2)
        ```
    ...
    def plot_permutation_importance(
        ...
        *,
        ...
        """Plota a importância por permutação das features dos modelos.
        ...
        experionml = ExperionMLClassifier(X, y, random_state=1)
        experionml.run(["LR", "RF"])
        experionml.plot_permutation_importance(show=10, n_repeats=7)
        ```
    ...
    def plot_pipeline(
        ...
        *,
        ...
        experionml = ExperionMLClassifier(X, y, random_state=1)
        experionml.run(["GNB", "RNN", "SGD", "MLP"])
        experionml.voting(models=experionml.winners[:2])
        experionml.plot_pipeline()

        experionml = ExperionMLClassifier(X, y, random_state=1)
        experionml.scale()
        experionml.prune()
        experionml.run("RF", n_trials=30)

        experionml.branch = "undersample"
        experionml.balance("nearmiss")
        experionml.run("RF_undersample")

        experionml.branch = "oversample_from_main"
        experionml.balance("smote")
        experionml.run("RF_oversample")
        ...
        def get_length(pl, i):
            """Obtém o comprimento máximo do nome de um bloco."""
            if len(pl) > i:
        ...
        def check_y(xy):
            """Retorna y, a menos que exista algo à direita; nesse caso, salta."""
        ...
        def add_wire(x, y):
            """Desenha um fio de conexão entre dois estimadores."""
            d.add(
    ...
    def plot_probabilities(
        ...
        *,
        ...
        """Plota a distribuição de probabilidade das classes alvo.
    ...
    def plot_residuals(
        ...
        *,
        ...
        """Plota os resíduos de um modelo.
    ...
    def plot_results(
        ...
        *,
        ...
        """Compara os resultados das métricas dos modelos.
        ...
        experionml = ExperionMLClassifier(X, y, random_state=1)
        experionml.run(["GNB", "LR"], metric=["f1", "recall"])
        experionml.plot_results()

        # Plot the time it took to fit the models
        experionml.plot_results(metric="time_fit+time")

        # Plot a different metric
        experionml.plot_results(metric="accuracy")

        # Plot the results on the training set
        experionml.plot_results(metric="f1", rows="train")
        ```
    ...
    def plot_roc(
        ...
        *,
        ...
        """Plota a curva Receiver Operating Characteristics.
    ...
    def plot_successive_halving(
        ...
        *,
        ...
        """Plota as pontuações por iteração do successive halving.
        ...
        experionml = ExperionMLClassifier(X, y, random_state=1)
        experionml.successive_halving(["Tree", "Bag", "RF", "LGB"], n_bootstrap=5)
    ...
    def plot_threshold(
        ...
        *,
        ...
        """Plota o desempenho das métricas em função dos valores de limiar.
