...
from experionml.utils.utils import check_is_fitted
...
def pipeline():
    ...
    experionml.encode()
    experionml.prune()
    experionml.add(StandardScaler(), columns=0)
    experionml.run("LR")

    def get_pipeline(model):
        return experionml.export_pipeline(model="LR" if model else None)
...
def pipeline_ts():
    ...
    experionml.run("NF")
    return experionml.export_pipeline(model="NF")
