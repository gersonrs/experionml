...
from experionml.utils.types import EngineTuple
...
def test_crash_with_logger():
    ...
    experionml = ExperionMLClassifier(X_bin, y_bin, logger="log")
    pytest.raises(RuntimeError, experionml.run, "LR", est_params={"test": 2})
...
def test_text_to_corpus():
    ...
    assert experionml.X.columns == ["corpus"]
...
def test_int_columns_to_str():
    ...
    assert experionml.X.columns[0] == "0"
...
def test_sparse_matrices_X_y():
    ...
    assert isinstance(experionml.X, pd.DataFrame)
    assert experionml.shape == (10, 4)
    assert experionml[experionml.columns[0]].dtype.name == "Sparse[int64, 0]"


def test_sparse_matrices_2_tuples():
    """Assert that sparse matrices are accepted as 2-tuples input."""
    experionml = ExperionMLClassifier((X_sparse, y10), (X_sparse, y10), random_state=1)
    assert isinstance(experionml.X, pd.DataFrame)
    assert experionml.shape == (20, 4)
    assert experionml[experionml.columns[0]].dtype.name == "Sparse[int64, 0]"
...
def test_inherit_sp():
    ...
    assert experionml.bats.estimator.get_params()["sp"] == 12  # Single seasonality
    assert experionml.tbats.estimator.get_params()["sp"] == [12, 24]  # Multiple seasonality
...
@patch("experionml.basetransformer.getLogger")
def test_log(cls):
