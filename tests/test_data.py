...
from experionml import ExperionMLClassifier, ExperionMLRegressor
...
def test_data_property():
    ...
    assert experionml._branches["main"]._container is None  # Inactive branch
    assert experionml._branches["main"]._data is not None  # Loads the data
...
def test_name_setter():
    ...
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    experionml.branch.name = "b1"
    assert experionml.branch.name == "b1"
...
def test_holdout_property():
    ...
    experionml.scale()
    assert not experionml.holdout.equals(experionml.branch.holdout)


def test_X_property():
    """Assert that the X property returns the feature set."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    assert experionml.branch.X.shape == (len(X_bin), X_bin.shape[1])


def test_y_property():
    """Assert that the y property returns the target column."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    assert experionml.branch.y.shape == (len(y_bin),)
...
def test_all_property():
    ...
    assert len(experionml.branch.dataset) != len(X_bin)
    assert len(experionml.branch._all) == len(X_bin)
...
def test_train_setter():
    ...
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    experionml.train = experionml.train.iloc[:100, :]
    assert experionml.train.shape == (100, X_bin.shape[1] + 1)


def test_test_setter():
    """Assert that the test setter changes the test set."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    experionml.test = experionml.test.iloc[:100, :]
    assert experionml.test.shape == (100, X_bin.shape[1] + 1)


def test_X_setter():
    """Assert that the X setter changes the feature set."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    experionml.X = experionml.X.iloc[:, :10]
    assert experionml.X.shape == (len(X_bin), 10)


def test_y_setter():
    """Assert that the y setter changes the target column."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    assert experionml.y[0] == 0  # The first value is 1 in original
    experionml.y = [1, *y_bin.values[1:]]
    assert experionml.y[0] == 1  # First value changed to 0


def test_X_train_setter():
    """Assert that the X_train setter changes the training feature set."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    new_X_train = experionml.X_train
    new_X_train.iloc[0, 0] = 999
    experionml.X_train = new_X_train.to_numpy()  # To numpy to test dtypes are maintained
    assert experionml.X_train.iloc[0, 0] == 999
    assert list(experionml.X_train.dtypes) == list(experionml.X_test.dtypes)


def test_X_test_setter():
    """Assert that the X_test setter changes the test feature set."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    new_X_test = experionml.X_test
    new_X_test.iloc[0, 0] = 999
    experionml.X_test = new_X_test
    assert experionml.X_test.iloc[0, 0] == 999


def test_y_train_setter():
    """Assert that the y_train setter changes the training target column."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    assert experionml.y_train.iloc[0] == 0  # The first value is 1 in original
    experionml.y_train = [1, *experionml.y_train[1:]]
    assert experionml.y_train.iloc[0] == 1  # First value changed to 0


def test_y_test_setter():
    """Assert that the y_test setter changes the training target column."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    assert experionml.y_test.iloc[0] == 1  # The first value is 0 in original
    experionml.y_test = [0, *experionml.y_test[1:]]
    assert experionml.y_test.iloc[0] == 0  # First value changed to 1


def test_data_properties_to_df():
    """Assert that the data attributes are converted to a df at setter."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    experionml.X = X_bin_array
    assert isinstance(experionml.branch.X, pd.DataFrame)


def test_data_properties_to_series():
    """Assert that the data attributes are converted to a series at setter."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    experionml.y = y_bin_array
    assert isinstance(experionml.branch.y, pd.Series)
...
def test_get_rows_is_dataframe():
    ...
    assert len(experionml.branch._get_rows(rows=experionml.test)) == len(experionml.test)


def test_get_rows_is_index():
    """Assert that an Index object returns the rows directly."""
    experionml = ExperionMLClassifier(X_idx, y_idx, index=True, random_state=1)
    assert len(experionml.branch._get_rows(rows=experionml.test.index)) == len(experionml.test)


def test_get_rows_is_range():
    """Assert that a range of rows is returned."""
    experionml = ExperionMLClassifier(X_idx, y_idx, index=True, random_state=1)
    assert len(experionml.branch._get_rows(rows=range(20, 40))) == 20


def test_get_rows_is_slice():
    """Assert that a slice of rows is returned."""
    experionml = ExperionMLClassifier(X_idx, y_idx, index=True, random_state=1)
    assert len(experionml.branch._get_rows(rows=slice(20, 100, 2))) == 40


def test_get_rows_by_exact_match():
    """Assert that name can select a row."""
    experionml = ExperionMLClassifier(X_idx, y_idx, index=True, random_state=1)
    assert experionml.branch._get_rows(rows="index_23").index[0] == "index_23"
...
def test_get_rows_by_str():
    ...
    assert len(experionml.branch._get_rows(rows="!index_3.*")) == len(X_idx) - 111
...
def test_get_columns_is_None():
    ...
    assert len(experionml.branch._get_columns(columns=None)) == 5
    assert len(experionml.branch._get_columns(columns=None, only_numerical=True)) == 4
    assert len(experionml.branch._get_columns(columns=None, include_target=False)) == 4


def test_get_columns_by_dataframe():
    """Assert that a dataframe retrieves columns."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    assert len(experionml.branch._get_columns(columns=experionml.X_train)) == X_bin.shape[1]


def test_get_columns_by_segment():
    """Assert that a range or slice retrieves columns."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    assert len(experionml.branch._get_columns(columns=slice(2, 6))) == 4
    assert len(experionml.branch._get_columns(columns=range(2, 6))) == 4
...
def test_get_columns_by_str():
    ...
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    assert len(experionml.branch._get_columns("mean radius+mean texture")) == 2
    assert len(experionml.branch._get_columns(["mean radius+mean texture", "mean area"])) == 3
...
def test_get_columns_by_type():
    ...
    experionml = ExperionMLClassifier(X10_str, y10, random_state=1)
    assert len(experionml.branch._get_columns(columns="number")) == 4
    assert len(experionml.branch._get_columns(columns="!number")) == 1
...
def test_get_columns_remove_duplicates():
    ...
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    assert experionml.branch._get_columns(columns=[0, 1, 0]) == ["mean radius", "mean texture"]
...
def test_get_target_column():
    ...
    assert experionml.branch._get_target(target="c", only_columns=True) == "c"
    assert experionml.branch._get_target(target=1, only_columns=True) == "b"
...
def test_get_target_class():
    ...
    experionml.clean()
    assert experionml.branch._get_target(target="y")[1] == 1
    assert experionml.branch._get_target(target=0)[1] == 0
...
def test_get_target_tuple_no_multioutput():
    ...
    with pytest.raises(ValueError, match=".*only accepted for multioutput tasks.*"):
        experionml.branch._get_target(target=(2, 1))


def test_get_target_tuple():
    """Assert that the target column and class can be retrieved."""
    experionml = ExperionMLClassifier(X_class, y=y_multiclass, random_state=1)
    assert experionml.branch._get_target(target=(2,)) == (2, 0)
    assert experionml.branch._get_target(target=("a", 2)) == (0, 2)
...
def test_load_store():
    ...
    experionml.branch = "2"
    assert experionml._branches["main"]._container is None  # Stored
    assert experionml._branches["main"].load(assign=False) is not None  # Loaded
    experionml._branches["main"].load(assign=True)
    assert experionml._branches["main"]._container is not None
...
def test_check_scaling_scaler_in_pipeline():
    ...
    assert not experionml.branch.check_scaling()
    experionml.add(MinMaxScaler())
    assert experionml.branch.check_scaling()


def test_check_scaling_sparse():
    """Assert that the sparse datasets return False for check_scaling."""
    experionml = ExperionMLClassifier(X_sparse, y=y10, random_state=1)
    assert not experionml.branch.check_scaling()
...
def test_check_scaling():
    ...
    experionml = ExperionMLClassifier(X_bin, y=y_bin, random_state=1)
    experionml.add(scaler)
    assert experionml.branch.check_scaling()


def test_check_scaling_drop_binary():
    """Assert that binary rows are dropped to check scaling."""
    experionml = ExperionMLClassifier(np.tile(y10, (10, 1)), y=y10, random_state=1)
    assert experionml.branch.check_scaling()
...
def test_og():
    ...
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    assert experionml._branches.og is experionml._branches["main"]
    experionml.scale()
    assert experionml._branches.og is experionml._branches._og


def test_current():
    """Assert that the current property returns the active branch."""
    experionml = ExperionMLClassifier(X_bin, y_bin, random_state=1)
    assert experionml._branches.current is experionml._branches["main"]
    experionml.branch = "b2"
    assert experionml._branches.current is experionml._branches["b2"]
...
def test_add_og():
    ...
    experionml = ExperionMLClassifier(X_bin, y_bin, memory="", random_state=1)
    experionml.scale()
    assert experionml._branches.og.name == "og"
    assert experionml._branches.og._container is None
    assert experionml._branches.og._data is not None
    assert glob.glob("joblib/experionml/Branch(og).pkl")
...
def test_add_store_previous_branch():
    ...
    experionml.scale()
    experionml.branch = "b2"
    assert experionml._branches["main"]._container is None
    assert experionml._branches.current is experionml._branches["b2"]
...
def test_add_copy_from_parent():
    ...
    experionml.scale()
    experionml.branch = "b2"
    assert_frame_equal(experionml.dataset, experionml._branches["main"].dataset)
    assert experionml.pipeline is not experionml._branches["main"].pipeline
    assert experionml.pipeline.steps[0][1] is experionml._branches["main"].pipeline.steps[0][1]
    assert experionml.branch._mapping is not experionml._branches["main"]._mapping
    assert experionml.standard_ is experionml._branches["main"].standard_
...
def test_holdout_is_same():
    ...
    experionml = ExperionMLClassifier(X_bin, y_bin, holdout_size=0.1, random_state=1)
    experionml.branch = "b2"
    assert experionml.holdout is experionml._branches["b2"].holdout
...
def test_reset():
    ...
    experionml.scale()
    experionml.branch = "b2"
    assert experionml.og is not experionml.branch
    assert len(experionml._branches) == 2
    assert glob.glob("joblib/experionml/Branch(main).pkl")
    experionml._branches.reset(hard=True)
    assert len(experionml._branches) == 1
    assert not glob.glob("joblib/experionml/Branch(main).pkl")
    assert experionml.og is experionml.branch
...
def test_numpy_engine():
    ...
    experionml = ExperionMLClassifier(X_bin, y_bin, engine="numpy", random_state=1)
    assert isinstance(experionml.dataset, np.ndarray)


def test_pandas_numpy_engine():
    """Assert that the pandas engine returns numpy dtypes."""
    experionml = ExperionMLClassifier(X_bin, y_bin, engine="pandas", random_state=1)
    assert all(isinstance(dtype, np.dtype) for dtype in experionml.dataset.dtypes)
    assert isinstance(experionml.y.dtype, np.dtype)


def test_pandas_pyarrow_engine():
    """Assert that the pandas-pyarrow engine returns pyarrow dtypes."""
    experionml = ExperionMLClassifier(X_bin, y_bin, engine="pandas-pyarrow", random_state=1)
    assert all(isinstance(dtype, pd.ArrowDtype) for dtype in experionml.dataset.dtypes)
    assert isinstance(experionml.y.dtype, pd.ArrowDtype)
...
def test_polars_engine():
    ...
    assert isinstance(experionml.X, pl.DataFrame)
    assert isinstance(experionml.y, pl.Series)


def test_polars_lazy_engine():
    """Assert that the polars-lazy engine returns polars types."""
    experionml = ExperionMLClassifier(X_bin, y_bin, engine="polars-lazy", random_state=1)
    assert isinstance(experionml.X, pl.LazyFrame)
    assert isinstance(experionml.y, pl.Series)
...
def test_pyarrow_engine():
    ...
    assert isinstance(experionml.X, pa.Table)
...
def test_modin_engine():
    ...
    assert "DataFrame" in str(experionml.X)
    assert "Series" in str(experionml.y)


def test_dask_engine():
    """Assert that the dask engine returns dask types."""
    experionml = ExperionMLClassifier(X_bin, y_bin, engine="dask", random_state=1)
    assert isinstance(experionml.X, dd.DataFrame)
    assert isinstance(experionml.y, dd.Series)
...
def test_pyspark_engine():
    ...
    experionml = ExperionMLClassifier(X_bin, y_bin, engine="pyspark", random_state=1)
    assert "createDataFrame" in str(experionml.X)
...
def test_pyspark_pandas_engine():
    ...
    experionml = ExperionMLClassifier(X_bin, y_bin, engine="pyspark-pandas", random_state=1)
    assert "DataFrame" in str(experionml.X)
    assert "Series" in str(experionml.y)
